...
small_sentence = sentence.lower().split()

for i in range(len(small_sentence)):
    mx1 = []
    for j in range(len(small_sentence[i])):
        if small_sentence[i][j] not in ".,;():-?!":
            mx1 += [small_sentence[i][j]]
    small_sentence[i] = ''.join(mx1)

print(*sorted(set(small_sentence)))
